...
class CoNSeP(Dataset):
    ...
    def __getitem__(self, idx):
        if self.train:
            ...
            label_dir = os.path.join(self.root_dir, self.img_id[idx], 'label.npy')

            img = imageio.imread(img_dir).astype(np.uint8)

            # mask = imageio.imread(mask_dir, as_gray=True).astype(np.bool)

            mask = np.load(label_dir)
...
def get_train_valid_loader(root_dir, batch_size=16, split=True,
    ...
    if split:
        ...
        train_transformed_dataset = CoNSeP(root_dir=root_dir,
                                                   img_id=train_id,
                                                   train=True,
        ...
        val_transformed_dataset = CoNSeP(root_dir=root_dir,
                                                 img_id=val_id,
                                                 train=True,
                                                 transform=transforms.Compose([
        ...
        transformed_dataset = CoNSeP(root_dir=root_dir,
                                             img_id=img_id,
                                             train=True,
                                             transform=transforms.Compose([
                                                 RandomCrop(256),
